...
│   ├── voiceAdd.ts           ← transcriptToTask(): speech transcript → Task via nlParser
│   ├── context/TaskContext.tsx  ← global task state (tasks, filePath, reload, save, pendingDateJump)
│   ├── hooks/                ← useVoiceAdd, voiceAddCommit, usePendingDone
│   └── components/           ← TaskRow, EventPill, StatsCard, PriorityPicker, RecurrencePicker,
│                               AddTaskModal, BottomActionBar, ViewSwitcher, VoiceAddButton
└── tests/
```
...
**Surfacing storage errors**: in **local mode**, `readTasks` (`mobile/src/store.ts`) swallows read failures and returns `[]` (a missing file is a normal, expected state — nothing has ever been saved there yet). In **iCloud mode**, `readTasks` never does this — every failure throws, including a missing file, because `enableICloudStorage` always creates the file, so a missing file in iCloud mode always means something went wrong (moved/deleted on another device, iCloud signed out, stale bookmark), never "nothing saved yet." `writeTasks` throws a descriptive `Error` on write failure in both modes. `TaskContext.reload` catches errors into `error: string | null` state (and also tracks read success in a separate `lastReadOkRef`, so a failed read can never cause `enableICloud`/`disableICloud` to overwrite a real file with an empty in-memory snapshot); `RootLayout` renders an `ErrorBanner` (`mobile/app/_layout.tsx`) above the `BottomActionBar` whenever `error` is non-null.
...
**Voice Add** (`mobile/src/hooks/useVoiceAdd.ts`, `voiceAddCommit.ts`, `mobile/src/voiceAdd.ts`, `mobile/src/components/VoiceAddButton.tsx`): a hold-to-talk `●`/`■` mic button sits in `BottomActionBar` between Search and the `+` add button. Press-and-hold (`onPressIn`/`onPressOut`) starts/stops on-device speech recognition via `expo-speech-recognition` (`requiresOnDeviceRecognition: true` — no network round-trip, works offline). `transcriptToTask()` (`voiceAdd.ts`) feeds the raw transcript straight through `nlParser.parseNaturalLanguage` — the same natural-language parser `AddTaskModal` uses — so "call dentist tomorrow" or "call dentist (A)" parses `start:`/priority exactly like typed input; it's fed directly to `parseLine` rather than through `applyAdd`/`buildAddRaw`, since `nlParser`'s `raw` already prepends `todayStr` and appends `start:` itself, and routing it through those again would double-prefix the creation date. `commitVoiceTranscript`/`undoVoiceTask` (`voiceAddCommit.ts`) are kept free of any `react-native` import — same pattern as `pendingDoneCommit.ts` — so they stay plain-Node-testable without an RN component-render setup. On commit, `VoiceAddToast` shows "Added: `<title>`" with a 5s-window UNDO (`undoVoiceTask` removes the task by matching `raw` string, mirroring `usePendingDone`'s raw-keyed-not-line-keyed pattern for the same reason: line numbers shift on any list mutation). The actual task append/commit happens on the `'end'` speech-recognition event, not in `stop()` — recognition finalizes its transcript asynchronously on the native side, so `stop()` only requests the session end while `'end'` is the real signal the transcript is ready. `sessionActiveRef` guards against a stray `'end'` firing for a session that never actually started (e.g. permission denied) and committing stale/empty transcript text. Requires two Info.plist permission strings wired via the `expo-speech-recognition` config plugin in `app.json` (`microphonePermission`, `speechRecognitionPermission`) — both must be present for `requestPermissionsAsync()` to succeed on-device, and both trigger the iOS permission prompt on first use only.
