...
**iCloud sync**: user points Settings file path to iCloud Drive container; iOS syncs automatically with other devices and with the CLI on Mac. On first launch (no saved config), the app defaults to `LOCAL_PATH` (the app's own Documents directory — always writable, no entitlement required). The Settings screen exposes LOCAL / iCLOUD preset buttons; the iCLOUD preset uses `ICLOUD_PATH` (`file:///private/var/mobile/Library/Mobile%20Documents/iCloud~com~caritos~todo-txt/Documents/todo.txt`) which requires the iCloud container `iCloud.com.caritos.todo-txt` to be created in the Apple Developer Portal and the `com.apple.developer.icloud-container-identifiers` entitlement in `app.json`. `writeTasks()` calls `makeDirectoryAsync({ intermediates: true })` before writing directly to the file path, so the parent directory is always created on demand. Writing directly (no tmp+rename) avoids an iCloud restriction that blocks creating `.tmp` files in the sync directory. `readConfig()` includes a migration that detects the old broken `Documents/../Library/Mobile Documents/` path pattern (which iOS rejects as non-writable) and resets it to `LOCAL_PATH`. `writeTasks()` wraps native write errors in a user-friendly message that includes the path and directs the user to Settings.
