...
**Goal:** Add a `/timeline` screen showing all 7 days of the current week as columns in a scrollable time grid, with timed tasks as absolutely-positioned pills.

**Architecture:** Single new screen `mobile/app/timeline.tsx` that reads tasks from `useTasks()`, groups them by day/timed status, and renders a 960px `ScrollView` with absolutely-positioned pill views per column. Three navigation files get one-line additions each.

**Tech Stack:** Expo SDK 52, Expo Router v3, React Native (iOS), `@shared/utils.addDays`, `@shared/parser.Task`, `mobile/src/context/TaskContext.useTasks`
...
| Action | Path | Purpose |
|--------|------|---------|
| Create | `mobile/app/timeline.tsx` | The full 7-column week timeline screen |
| Modify | `mobile/app/_layout.tsx` | Register the `timeline` route |
| Modify | `mobile/src/components/ViewSwitcher.tsx` | Add Timeline entry between Week and Year |
| Modify | `mobile/src/components/BottomActionBar.tsx` | Add `/timeline` label |

---

## Task 1: Create `mobile/app/timeline.tsx`

**Files:**
- Create: `mobile/app/timeline.tsx`

No tests: this is a React Native screen component with no extractable business logic. All the logic (`taskTime`, `topOffset`, `cleanTitle`) mirrors `mobile/app/day/[date].tsx` exactly — those helpers are not tested there either.

- [ ] **Step 1: Create the file**

Create `mobile/app/timeline.tsx` with this complete implementation:
...
```bash
cd /Users/eladio/src/todo-txt/mobile && npx tsc --noEmit 2>&1 | head -30
```

Expected: no errors related to `timeline.tsx`. (Unrelated pre-existing errors are OK.)

- [ ] **Step 3: Commit**

```bash
git add mobile/app/timeline.tsx
git commit -m "feat: add Timeline 7-column week view screen"
```
...
In `mobile/app/_layout.tsx`, add `<Stack.Screen name="timeline" />` after `<Stack.Screen name="year" />`:
...
- [ ] **Step 2: Add Timeline to `ViewSwitcher.tsx`**

In `mobile/src/components/ViewSwitcher.tsx`, add the Timeline entry between Week and Year:
...
- [ ] **Step 3: Add Timeline label to `BottomActionBar.tsx`**

In `mobile/src/components/BottomActionBar.tsx`, add the route label:
...
git commit -m "feat: wire Timeline route into navigation"
```
...
After running the app via `mobile/scripts/sim.sh`:

- [ ] ViewSwitcher shows "Timeline" between "Week" and "Year"
- [ ] Navigating to Timeline shows the 7-column grid
- [ ] Bottom bar label reads "Timeline" on the timeline screen
- [ ] Today's column has the subtle accent tint (`#E8461A11`)
- [ ] Today's day number has accent square background, no border-radius
- [ ] Busy dots appear under day numbers that have tasks
- [ ] Timed tasks appear as pills at their correct hour offset in the right column
- [ ] All-day row appears only when at least one day has non-timed tasks
- [ ] All-day row hidden when no all-day tasks exist
- [ ] "nothing this week." shown when the week has no tasks at all
- [ ] ‹ › navigation moves to the previous/next week
- [ ] Current time line (red dot + bar) visible on today's week, hidden on other weeks
- [ ] ScrollView auto-scrolls to current hour − 2 on today's week; to 8 AM on other weeks
