...
New Objective-C native module (`RCT_EXPORT_MODULE()`, accessed via `NativeModules`, matching the existing project convention that `requireOptionalNativeModule` silently no-ops for local modules — see `mobile/CLAUDE.md`'s iCloud native-module notes, and matching the Objective-C style the earlier — now removed — iCloud module used). No new entitlements are required: this module only ever touches locations the user explicitly grants through the system document picker, which runs with its own (not the app's) filesystem access.

**Packaging:** `mobile/ios/` is gitignored and regenerated by `expo prebuild` (confirmed via `git ls-files`), so native source can't be hand-edited there directly. Following the established local-module pattern (see the global CLAUDE.md's "Local `file:` package.json dependencies" notes on this exact mechanism), the module lives at `mobile/modules/expo-icloud-file/` as its own tiny npm package (`package.json`, `ExpoIcloudFile.podspec`, `ios/ExpoIcloudFile.h`, `ios/ExpoIcloudFile.m`) referenced from `mobile/package.json` via `"expo-icloud-file": "file:./modules/expo-icloud-file"`. React Native's classic autolinking (`use_native_modules!` in the generated `Podfile` — already present alongside `use_expo_modules!`) picks up its podspec automatically on the next `pod install`, which `mobile/scripts/sim.sh` already triggers via its full clean rebuild whenever `Podfile.lock` is stale.

- **`pickFolder(sourcePath: string) -> Promise<{ bookmark: string, name: string }>`**
  Presents `UIDocumentPickerViewController` in export mode for the file at `sourcePath` (a temp local file the JS side writes first with current tasks serialized, reusing the existing `handleExport` temp-file pattern in `settings.tsx`). No `directoryURL` hint is set — that would require re-declaring the `com.apple.developer.ubiquity-container-identifiers` entitlement and regenerating the App Store Connect provisioning profile (the documented manual process in the root CLAUDE.md), which isn't worth it for a one-time convenience: iOS's document picker already remembers the last-used destination automatically across invocations, so only the very first setup opens at the default top-level location. Settings copy suggests a folder name ("Stark") so the user just taps "New Folder," types it, and confirms. The user chooses or creates a destination folder, the temp file is copied in as `todo.txt`, and a security-scoped bookmark for the destination folder is created (`[NSURL bookmarkDataWithOptions:...]`) and returned base64-encoded along with a display name.
...
- **Local mode:** a single `USE ICLOUD DRIVE` button with description text suggesting the folder name (e.g. "Choose or create a folder in iCloud Drive — we suggest naming it 'Stark'."). Tapping it calls `enableICloudStorage(tasks)`, which opens the export picker (default starting location — see Native Module section), then `reload()`. On picker cancellation, no-op (no error alert — cancellation isn't a failure). On other failures, `Alert.alert('Could not enable iCloud Drive', message)`.
